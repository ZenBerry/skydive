(function () {
  ...
  function createExplicitYGraph(equation) {
    ...
    return {
      type: equation.type,
      paths: points.length ? [points] : [],
      wave: fillMissingValues(normalizeValues(values))
    };
  }

  function createExplicitXGraph(equation) {
    const points = [];
    const valuesByX = new Array(SAMPLE_COUNT).fill(NaN);
    for (let index = 0; index < SAMPLE_COUNT; index += 1) {
      const phase = index / (SAMPLE_COUNT - 1);
      ...
      if (Number.isFinite(x)) {
        points.push({ x, y });
        const sampleIndex = Math.round((x - GRAPH_MIN_X) / (GRAPH_MAX_X - GRAPH_MIN_X) * (SAMPLE_COUNT - 1));
        if (sampleIndex >= 0 && sampleIndex < SAMPLE_COUNT) valuesByX[sampleIndex] = y;
      }
    }
    return {
      type: equation.type,
      paths: points.length ? [points] : [],
      wave: fillMissingValues(normalizeValues(valuesByX))
    };
  }
  ...
  function findImplicitY(equation, x, preferredY) {
    const steps = 64;
    const roots = [];
    let previousY = GRAPH_MIN_Y;
    let previousValue = evaluate(equation.fn, x, previousY, x, 0);
    for (let index = 1; index <= steps; index += 1) {
      const y = GRAPH_MIN_Y + (GRAPH_MAX_Y - GRAPH_MIN_Y) * index / steps;
      const value = evaluate(equation.fn, x, y, x, index / steps);
      if (Number.isFinite(previousValue) && Number.isFinite(value)) {
        if (Math.abs(value) < 0.00001) roots.push(y);
        if ((previousValue <= 0 && value >= 0) || (previousValue >= 0 && value <= 0)) {
          ...
          for (let pass = 0; pass < 18; pass += 1) {
            ...
            const midValue = evaluate(equation.fn, x, midY, x, index / steps);
            if (!Number.isFinite(midValue)) break;
            if ((loValue <= 0 && midValue >= 0) || (loValue >= 0 && midValue <= 0)) {
              hiY = midY;
            } else {
              loY = midY;
              loValue = midValue;
    ...
    const target = Number.isFinite(preferredY) ? preferredY : 0;
    return roots.sort((a, b) => Math.abs(a - target) - Math.abs(b - target))[0];
  }
  ...
  function createImplicitGraph(equation) {
    ...
    for (let index = 0; index < SAMPLE_COUNT; index += 1) {
      const phase = index / (SAMPLE_COUNT - 1);
      const x = GRAPH_MIN_X + (GRAPH_MAX_X - GRAPH_MIN_X) * phase;
      const y = findImplicitY(equation, x, preferredY);
      if (Number.isFinite(y)) preferredY = y;
      values.push(y);
    }
  ...
  window.SkydiveCommands.push({
    id: "flufi",
    aliases: ["graph-equation", "equation-synth"],
    title: "Flufi",
    description: "Play explicit and implicit equation graphs from the keyboard.",
    acceptsArgs: true,
    ...
    getTitle(state) {
      ...
      return formula ? `Flufi ${formula}` : "Flufi";
    },
    ...
    render(container, state, updateState) {
      ...
      container.innerHTML = `
        <div class="flufi-card" data-active="false" data-playing="false">
          <div class="flufi-head">
            <div class="flufi-title">Flufi</div>
            <div class="flufi-mode" aria-live="polite">${graph.type === "implicit" ? "implicit" : graph.type === "explicit-x" ? "x=f(y)" : "y=f(x)"}</div>
            <div class="flufi-key" aria-live="polite"></div>
          </div>
          <canvas class="flufi-graph" aria-label="Flufi equation graph"></canvas>
          <input class="flufi-formula" data-command-interactive spellcheck="false" aria-label="Flufi equation">
          <label class="flufi-speed-row">
            <span class="flufi-speed-label">Loop speed</span>
            <input class="flufi-speed" data-command-interactive type="range" min="${MIN_SPEED_EXPONENT}" max="${MAX_SPEED_EXPONENT}" step="0.05" aria-label="Flufi loop speed">
            <span class="flufi-speed-value"></span>
          </label>
          <div class="flufi-actions">
            <button class="flufi-button" type="button" data-command-interactive data-action="activate">Activate keyboard</button>
          </div>
          <div class="flufi-error" aria-live="polite"></div>
        </div>
      `;
      ...
      style.textContent = `
        ...
        .flufi-card[data-active="true"] .flufi-button {
          color: #5b38ac;
        }
      ...
      const card = container.querySelector(".flufi-card");
      const canvas = container.querySelector(".flufi-graph");
      const input = container.querySelector(".flufi-formula");
      const speedInput = container.querySelector(".flufi-speed");
      const speedValue = container.querySelector(".flufi-speed-value");
      const button = container.querySelector('[data-action="activate"]');
      const errorNode = container.querySelector(".flufi-error");
      const activeKey = container.querySelector(".flufi-key");
      input.value = formula;
